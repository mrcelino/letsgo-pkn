...
export default function ThreeScene({
    ...
    try {
      ...
      fallbackDiv.innerHTML = `
        <div style="max-width: 500px;">
          ...
          <h2 style="font-size: 1.5rem; margin-bottom: 1rem; color: #D4AF37;">WebGL Tidak Tersedia</h2>
          <p style="color: #aaa; line-height: 1.6; margin-bottom: 1.5rem;">
            Browser Anda tidak mendukung WebGL atau terjadi masalah dengan rendering 3D.<br>
            Peta interaktif memerlukan dukungan WebGL untuk berfungsi.
          </p>
          <div style="background: rgba(212,175,55,0.1); border: 1px solid rgba(212,175,55,0.3); border-radius: 8px; padding: 1rem; margin-bottom: 1.5rem;">
            <p style="font-size: 0.875rem; color: #D4AF37; margin: 0;">
              <strong>Solusi:</strong><br>
              • Refresh halaman (Ctrl/Cmd + F5)<br>
              • Update browser ke versi terbaru<br>
              • Aktifkan Hardware Acceleration<br>
  ...
  useEffect(() => {
    ...
    const animate = () => {
      // Safety check - stop animation if renderer is not available
      ...
      if (mapGroupRef.current && cameraRef.current) {
        ...
        if (currentIsMapMode) {
          ...
          mapGroupRef.current.rotation.x +=
            (0 - mapGroupRef.current.rotation.x) * 0.05;
        ...
        if (intersects.length > 0) {
          ...
          if (mesh && mesh !== hoveredObjectRef.current) {
            ...
            if (hoveredObjectRef.current) {
              (
                hoveredObjectRef.current.material as THREE.MeshStandardMaterial
              ).color.setHex(hoveredObjectRef.current.userData.originalColor);
              (
                hoveredObjectRef.current.material as THREE.MeshStandardMaterial
              ).emissiveIntensity = 0.2;
  ...
  return (
    <div
      ref={containerRef}
      className={`fixed top-0 left-0 w-full h-full z-0 transition-opacity duration-1000 touch-none ${
        isMapMode ? "opacity-100" : "opacity-80"
      }`}
    />
  );
}
